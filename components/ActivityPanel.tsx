          ...
          <div key={item.id} className={`p-3 rounded-lg border text-sm relative group transition-all ${item.status === 'needs_input' ? 'bg-amber-50/50 border-amber-200' : 'bg-white border-slate-100 shadow-sm'}`}>
              <div className="flex justify-between items-start mb-1">
                  <div className="flex items-center gap-2">
                      {icon}
                      <span className="font-semibold text-slate-700 truncate max-w-[180px]" title={item.name}>{item.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        <div className="flex-1 overflow-hidden flex flex-col bg-slate-50">
            ...
            <div className="flex-1 overflow-y-auto p-3">
                <div className="flex justify-between items-center mb-2">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">History</span>
