...
- **ID 1: WCVP Baseline.** The immutable 1.4M record core.
- **ID 2: FloraCatalog System (Derived).** Backbone records like 'Family' or 'Order' generated by internal scripts. These inherit their scientific authority from the WCVP attributes (specifically the 'family' column) from which they were derived.
- **ID 3: User Manual Entry.** Records added via the UI or bulk file upload that do not yet exist in an authoritative checklist.
...
*   **Example Rows:**
    ...
    *   ID: 2, Name: "FloraCatalog System", Version: "v2.31.1 (Derived)", Context: "Derived from WCVP family attribute"
