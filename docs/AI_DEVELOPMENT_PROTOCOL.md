...
- **Strict Line-by-Line Preservation**: For files tagged as "Contextual Baseline" or "Roadmap" (like `TASK_BACKLOG.md`), the AI must copy the *entire* existing content exactly and only append or modify specific status markers. Re-summarizing or "cleaning up" sections from memory is strictly prohibited.
- **Diff Verification**: Before emitting XML, the AI must perform a mental diff between the proposed output and the source file provided in the prompt to ensure no tasks, descriptions, or priority sections were accidentally deleted.
- **Cross-Reference Validation**: The AI must verify that the number of tasks and sections in the proposed update matches the source file to prevent "memory-based summaries" from replacing real data.
- **Preservation Over Brevity**: In roadmap documents, maintaining a complete and accurate historical record is more important than keeping the XML response short.
