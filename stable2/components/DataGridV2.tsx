...
const DataGridV2: React.FC<DataGridProps> = ({ taxa, onAction, preferences }) => {
  ...
  const allColumns: ColumnConfig[] = [
      { id: 'treeControl', label: 'Tree', defaultWidth: 53, filterType: undefined, disableSorting: true, lockWidth: true }, 
      { id: 'actions', label: 'Actions', defaultWidth: 70, filterType: undefined, disableSorting: true, lockWidth: true }, 
      ...
      <div className="p-2 border-b border-slate-200 bg-slate-50 flex justify-between items-center z-20 relative flex-shrink-0">
         ...
         <div className="flex items-center gap-2">
             ...
             <button 
                ...
                className={`p-1.5 border rounded shadow-sm transition-all mr-2 ${isHierarchyMode ? 'bg-indigo-100 text-indigo-700 border-indigo-300 shadow-inner' : 'bg-white text-slate-400 border-slate-300 hover:text-slate-600 hover:bg-slate-50'}`}
                      ...
                      >
                         <div className={`flex items-center gap-1 p-2 h-full w-full ${col.headerAlign === 'center' ? 'justify-center' : 'justify-between'}`}>
                             <div className="flex items-center gap-1 cursor-grab active:cursor-grabbing overflow-hidden" onClick={() => !col.disableSorting && handleSort(col.id)}>
                                {col.id !== 'actions' && col.id !== 'treeControl' && !col.disableDrag && !col.hideHeaderIcons && <GripVertical size={12} className="text-slate-300 opacity-0 group-hover:opacity-100 cursor-move flex-shrink-0" />}
                                <span className="truncate">
                                    {col.id === 'treeControl' ? <Network size={14} className="mx-auto text-slate-400"/> : col.label}
                                </span>
                             </div>
                             {col.id !== 'actions' && col.id !== 'treeControl' && !col.disableSorting && !col.hideHeaderIcons && (<button onClick={() => handleSort(col.id)} className="flex-shrink-0"><ArrowUpDown size={12} className={sortConfig?.key === col.id ? 'text-leaf-600' : 'text-slate-300 hover:text-slate-500'}/></button>)}
                         </div>
